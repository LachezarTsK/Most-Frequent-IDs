...
function mostFrequentIDs(IDs: number[], changeInFrequency: number[]): number[] {
    const maxID = Math.max(...IDs);
    const frequencyIDs: number[] = new Array(maxID + 1).fill(0);
    const resultMostFrequentIDsPerTimeStamp: number[] = new Array(IDs.length).fill(0);
    const maxHeap = new PriorityQueue<Component>((x, y) => y.frequency - x.frequency);
...
class Component {
    constructor(public ID: number, public frequency: number) {}
}
